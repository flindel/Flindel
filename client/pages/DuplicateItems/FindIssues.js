...
class FindIssues extends Component {
  ...
  componentDidMount() {
    ...
    fetch(
      `${api_name}/collections/count?id=${encodeURIComponent(
      ...
      if (response.ok) {
        var allProducts = response.json().count;
      } else {
        throw Error(response.statusText);
      }
    });
    let pages = allProducts / 250; //number of pages of products
    //Assumption, Brand has less than 250 products
    for (let i = 1; i <= pages + 1; i++) {
      fetch(
        `${api_name}/collections?id=${encodeURIComponent(
          collection_all_products_id
        )}&page=${encodeURIComponent(i)}`,
        {
          method: "GET"
        }
      )
        ...
        .then(resData => {
          this.setState(prevState => {
            all: resData.products.push(prevState.all);
          });
