...
class Blacklist extends Component {
  ...
  async doesProductExist(ID) {
    ...
    let temp = await fetch(
      `https://${serveoname}/products?shop=${encodeURIComponent(
    ...
    if (response) {
      ...
      let temp2 = await fetch(
        `https://${serveoname}/products/variant/exists?store=${encodeURIComponent(
          this.state.storeName
        )}&id=${encodeURIComponent(ID)}`,
        {
          method: "get"
    ...
    if (this.state.valid) {
      ...
      if (tempList.indexOf(toAdd) == -1) {
        ...
        if (productExists) {
          ...
          fetch(
            `https://${serveoname}/blacklist?items=${encodeURIComponent(
              itemString
  ...
  async getItems() {
    let temp = await fetch(
      `https://${serveoname}/blacklist?store=${encodeURIComponent(
        this.state.storeName
      )}`,
      {
        method: "get"
