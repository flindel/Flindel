...
class deliverOrders extends Component {
  ...
  async handleSubmit() {
    ...
    if (failedList.length > 0) {
      await fetch(
        `https://${serveoname}/fulfillment/update?orders=${encodeURIComponent(
          orders
        )}`,
        {
          method: "put"
    ...
    if (deliveredList.length > 0) {
      await fetch(
        `https://${serveoname}/fulfillment/complete?orders=${encodeURIComponent(
          orders
        )}`,
        {
          method: "post"
  ...
  async loadFulfillments() {
    ...
    let temp = await fetch(
      `https://${serveoname}/fulfillment/deliver?workerID=${encodeURIComponent(
        this.state.workerID
      )}`,
      {
        method: "get"
