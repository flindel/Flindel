...
class returnShipment extends Component {
  ...
  async confirmShipment() {
    let temp = await fetch(
      `https://${serveoname}/item/confirmDelivery?code=${encodeURIComponent(
        this.state.existing
      )}`,
      {
        method: "put"
  ...
  async isCodeUnique(code) {
    let temp = await fetch(
      `https://${serveoname}/item/code/unique?code=${encodeURIComponent(code)}`,
      {
        method: "get"
      }
    );
  ...
  sendEmails() {
    ...
    fetch(
      `https://${serveoname}/send/returnShipment?code=${encodeURIComponent(
        this.state.code
  ...
  updateDB() {
    ...
    for (var i = 0; i < this.state.returnList.length; i++) {
      ...
      fetch(
        `https://${serveoname}/item/returned?quantity=${encodeURIComponent(
          qty
  ...
  async loadItems() {
    let temp = await fetch(
      `https://${serveoname}/item/storeList?store=${encodeURIComponent(
        this.state.store
      )}`,
      {
        method: "get"
