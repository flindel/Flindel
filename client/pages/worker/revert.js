...
class Revert extends React.Component {
  ...
  async getFulfillmentService(shop = this.state.shop) {
    ...
    var temp = await fetch(
      `${api_name}/revert/fulserv/firestore/id?shop=${encodeURIComponent(
        shop
      )}`,
      {
        method: "get"
  ...
  del(product_id, shop, callback = doNothing) {
    fetch(
      `${api_name}/revert/products?id=${encodeURIComponent(
        product_id
  ...
  async getGitCollectionId(shop) {
    fetch(
      `${api_name}/revert/collections/all/?shop=${encodeURIComponent(shop)}`,
      {
        method: "get"
      }
    )
  ...
  deleteGitCollect(gitCollectionId, shop) {
    fetch(
      `${api_name}/revert/collections?id=${encodeURIComponent(
        gitCollectionId
      )}&shop=${encodeURIComponent(shop)}`,
      {
        method: "delete"
  ...
  deleteFulserv(fulservId, shop) {
    fetch(
      `${api_name}/revert/fulserv?id=${encodeURIComponent(
        fulservId
      )}&shop=${encodeURIComponent(shop)}`,
      {
        method: "delete"
  ...
  async revertScriptTag(shop) {
    ...
    let scripttagIDTemp = await fetch(
      `${api_name}/revert/scriptTag/db/ids?shop=${encodeURIComponent(shop)}`,
      {
        method: "get"
      }
    );
    ...
    for (let i = 0; i < idsArray.length; i++) {
      ...
      let deleteTemp = await fetch(
        `${api_name}/revert/scriptTag/shopify?id=${encodeURIComponent(
          idsArray[i]
        )}&shop=${encodeURIComponent(shop)}`,
        {
          method: "delete"
    ...
    let statusTemp = await fetch(
      `${api_name}/revert/scriptTag/db/status?shop=${encodeURIComponent(shop)}`,
      {
        method: "get"
      }
    );
