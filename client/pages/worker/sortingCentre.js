...
class sortingCentre extends Component {
  ...
  setOpenTime() {
    fetch(
      `https://${serveoname}/return/requested/openTime?code=${encodeURIComponent(
        this.state.cCode
      )}`,
      {
        method: "PUT"
  ...
  setCloseTime() {
    fetch(
      `https://${serveoname}/return/requested/closeTime?code=${encodeURIComponent(
        this.state.cCode
      )}`,
      {
        method: "PUT"
  ...
  async updateItems(items) {
    ...
    while (items.length > 0) {
      ...
      await fetch(
        `https://${serveoname}/return/pending/items?code=${encodeURIComponent(
          activeCode
  ...
  async addItem5() {
    ...
    if (productID != 0 && validInfo) {
      let GITinfo = await fetch(
        `https://${serveoname}/products/GITinformation?productID=${encodeURIComponent(
          productID
  ...
  async getItemInformation(varID, store) {
    let temp = await fetch(
      `https://${serveoname}/products/all?store=${encodeURIComponent(store)}`,
      {
        method: "get"
      }
    );
  ...
  async sendToDB() {
    ...
    await fetch(
      `https://${serveoname}/return/pending/new?code=${encodeURIComponent(
        this.state.cCode
      )}`,
      {
        method: "post"
  ...
  async handleSubmit2() {
    ...
    //update new reasons
    await fetch(
      `https://${serveoname}/return/requested/itemStatus?code=${encodeURIComponent(
        this.state.cCode
  ...
  async handleSubmit() {
    let temp = await fetch(
      `https://${serveoname}/return/requested/items?code=${encodeURIComponent(
        this.state.cCode
      )}`,
      {
        method: "get"
