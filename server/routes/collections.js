...
router.get("/", async ctx => {
  ...
  const option = {
    url: `https://${shop}/${api_link}/products.json?collection_id=${collectionid}&limit=250&page=${page}`,
    headers: {
...
router.get("/page/", async ctx => {
  //Gets products all products from a collection
  const page_info = ctx.query.page_info;
  const { cookies } = ctx;
  const shop = cookies.get("shop_id");
  const accessToken = cookies.get("accessToken");
  const option = {
    url: `https://${shop}/${api_link}/products.json?page_info=${page_info}&limit=250`,
    headers: {
      "X-Shopify-Access-Token": accessToken
    },
    json: true
...
router.get("/count", async ctx => {
  ...
  const accessToken = cookies.get("accessToken");
  const option = {
    url: `https://${shop}/${api_link}/products/count.json?collection_id=${collectionid}`,
    headers: {
      "X-Shopify-Access-Token": accessToken
    },
    json: true
